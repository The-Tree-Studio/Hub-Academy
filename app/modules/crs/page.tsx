...
const FICHES_FR = [
  ...
  },
  {
    ...
    "aretenir": "Sans auto-certification valide, le **PSF** ne peut pas finaliser l’entrée en relation. i-Hub vérifie la présence et la cohérence du document — et signale toute absence au PSF.",
    "plusLoin": [
      {
        "icon": "📋",
  ...
  },
  {
    ...
    "aretenir": "i-Hub ne peut pas ignorer un changement de circonstances visible. Toute mise à jour d’adresse est signalée au PSF qui décide de la reclassification CRS."
  },
  {
    "id": 12,
    "emoji": "⚖️",
...
const FICHES_EN = [
  ...
  },
  {
    ...
    "aretenir": "i-Hub cannot ignore a visible change of circumstances. Any address update is flagged to the PSF which decides on reclassification."
  },
  {
    "id": 12,
    "emoji": "⚖️",
...
const CAS_FR = [{"situation":"Un PSF signale qu’un client final belge Il refuse de fournir son auto-certification au PSF. Le PSF demande à i-Hub d’analyser le dossier.","action":"Signaler au PSF : sans auto-certification, le PSF ne peut pas finaliser l’entrée en relation","options":["Accepter le dossier sans auto-certification","Signaler au PSF : sans auto-certification, le PSF ne peut pas finaliser l’entrée en relation","Déclarer à l’ACD à l’ACD sans auto-certification","Appliquer FATCA à la place du CRS"],"explication":"L’auto-certification est obligatoire. Sans elle, le **PSF** ne peut pas finaliser l’entrée en relation. i-Hub signale cette anomalie au PSF — c’est le PSF qui prend la décision."},{"situation":"Un PSF transmet le dossier d’une holding luxembourgeoise dont un actionnaire français détient 30% du capital. Elle se déclare ENF Active.","action":"Signaler au PSF : ENF probablement Passive (30% > 25%), actionnaire français à vérifier","options":["Accepter — ENF Active déclarée par le client","Signaler au PSF : ENF probablement Passive (30% > 25%), actionnaire français à vérifier","Transmettre directement le contrôle à déclarer","Déclarer la holding comme résidente française"],"explication":"30% > 25% → probable ENF Passive. i-Hub signale l’incohérence au PSF. C’est le PSF qui reclassifie et décide des obligations de déclaration."},{"situation":"Un PSF vous informe qu’un client final déménage d’Allemagne en Thaïlande (non-participant CRS). Il soumet une nouvelle auto-certification.","action":"Signaler au PSF : nouvelle résidence en Thaïlande (non-CRS), à lui de mettre à jour la classification","options":["Continuer à signaler au PSF comme avant","Signaler au PSF : nouvelle résidence en Thaïlande (non-CRS), à lui de mettre à jour la classification","Déclarer à l’ACD quand même CRS","Appliquer FATCA à la place","Déclarer à l’Allemagne car il y résidait avant"],"explication":"La Thaïlande ne participe pas au CRS. Un résident thaïlandais n’est pas déclarable sous ce régime — sous réserve de vérification."},{"situation":"Un PSF signale qu’un client final a ouvert un compte le 1er mars 2025. Au 30 juin 2025, il n’a toujours pas fourni son auto-certification au PSF.","action":"Signaler au PSF : délai de 90 jours dépassé, le PSF doit agir","options":["Pas de problème — 30 juin est la date limite de déclaration","Accorder 90 jours supplémentaires","Signaler au PSF : délai de 90 jours dépassé, le PSF doit agir","Escalader au PSF : délai dépassé, décision à prendre"],"explication":"90 jours à partir du 1er mars 2025 = 30 mai. Au 30 juin le délai est dépassé. i-Hub signale l’anomalie au **PSF** qui doit prendre les mesures nécessaires avec son client final."},{"situation":"Un résident luxembourgeois d’origine américaine déclare une résidence fiscale aux USA uniquement dans son auto-certification.","action":"Vérifier : FATCA s’applique (US Person), CRS non — mais demander confirmation de non-résidence LU","options":["Appliquer CRS uniquement — pas besoin de FATCA","Ignorer — il est luxembourgeois","Vérifier : FATCA s’applique (US Person), CRS non — mais demander confirmation de non-résidence LU","Déclarer sous les deux régimes automatiquement"],"explication":"US Person → FATCA obligatoire. Résidence fiscale USA seulement → CRS non applicable. Mais vérifier la non-résidence LU."}]
const CAS_EN = [{"situation":"A PSF reports that a Belgian final client They refuse to provide their self-certification to the PSF. The PSF asks i-Hub to analyse the file.","action":"Flag to PSF: without self-certification, the PSF cannot finalise onboarding","options":["Accept the file without self-certification","Flag to PSF: without self-certification, the PSF cannot finalise onboarding","Report to ACD without self-certification","Apply FATCA instead of CRS"],"explication":"Self-certification is mandatory. Without it, the **PSF** cannot complete onboarding. i-Hub flags this anomaly to the PSF — it is the PSF that makes the decision."},{"situation":"A Luxembourg holding has a French shareholder holding 30% of capital. It declares itself an Active NFE.","action":"Flag to PSF: probable Passive NFE (30% > 25%), French shareholder to verify","options":["Accept — Active NFE as declared","Flag to PSF: probable Passive NFE (30% > 25%), French shareholder to verify","Report directly to ACD person to be reported","Report the holding as a French resident"],"explication":"A holding with mainly passive income is a Passive NFE. 30% > 25% → French shareholder must be reported."},{"situation":"A PSF informs you that a final client moves from Germany to Thailand (CRS non-participant). They submit a new self-certification.","action":"Flag to PSF: new residency in Thailand (non-CRS), PSF to update classification","options":["Continue flagging to PSF as before","Flag to PSF: new residency in Thailand (non-CRS), PSF to update classification","Report to ACD anyway CRS","Apply FATCA instead","Report to Germany as they previously resided there"],"explication":"Thailand does not participate in CRS. A Thai resident is not reportable under this regime — subject to verification."},{"situation":"A PSF reports that a final client opened an account on 1 March 2025. By 30 June 2025, they have still not provided their self-certification to the PSF.","action":"Flag to PSF: 90-day deadline exceeded, PSF must take action","options":["No problem — 30 June is the reporting deadline","Grant an additional 90 days","Flag to PSF: 90-day deadline exceeded, PSF must take action","Close the account immediately"],"explication":"90 days from 1 March 2025 = 30 May. By 30 June the deadline has passed. i-Hub flags the anomaly to the **PSF** which must take the necessary steps with its final client."},{"situation":"A Luxembourg resident of US origin declares US tax residence only in their self-certification.","action":"Check: FATCA applies (US Person), CRS does not — but confirm non-Luxembourg residence","options":["Apply CRS only — no need for FATCA","Ignore — they are Luxembourg-based","Check: FATCA applies (US Person), CRS does not — but confirm non-Luxembourg residence","Report under both regimes automatically"],"explication":"US Person → FATCA mandatory. Tax residence USA only → CRS not applicable. But verify non-Luxembourg tax residence."}]
